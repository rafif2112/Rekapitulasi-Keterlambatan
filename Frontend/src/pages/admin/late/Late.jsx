...
export default function Late() {
    ...
    useEffect(() => {
        ...
        axiosInstance.get('late/student')
            .then((response) => {
    ...
    const handleDelete = (id) => {
        axiosInstance.delete(`/late/delete/${id}`)
            .then((response) => {
                setLateRecords(lateRecords.filter(record => record.id !== id));
            })
            .catch((error) => {
                console.error(error);
            });
    ...
    const handleExport = () => {
        setLoading(true);
        axiosInstance.get('/export-admin', {
            responseType: 'blob'
        })
        .then((response) => {
    ...
    const handleDownloadPDF = (studentId) => {
        setLoading(true);
        axiosInstance.get(`/download-pdf/${studentId}`)
        .then((response) => {
            setLoading(false);
        })
        .catch((error) => {
                ...
                <div className="container mx-auto">
                    <div className="bg-white p-6 rounded-lg shadow-lg">
                        ...
                        <div className="flex space-x-2 mb-4">
                            <Link to={'create'} className="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 transition duration-300">Tambah</Link>
                            <button onClick={handleExport} className="bg-teal-500 text-white px-4 py-2 rounded hover:bg-teal-600 transition duration-300">Export Data Keterlambatan</button>
                        </div>
                                ...
                                <table className="min-w-full bg-white">
                                    <thead>
                                        <tr>
                                            ...
                                            <th className="py-2 px-4 border-b">Informasi</th>
                                        ...
                                        {loading ? (
                                            ...
                                            lateRecords.map((lateRecord, index) => (
                                                <tr key={lateRecord.id}>
                                                    ...
                                                    <td className="py-2 px-4 border-b text-center">{lateRecord.student_id}</td>
                                                    <td className="py-2 px-4 border-b text-center">{lateRecord.date_time_late}</td>
                                                    <td className="py-2 px-4 border-b text-center">{lateRecord.information}</td>
                                                    <td className="py-2 px-4 border-b text-center">
                                                        <Link to={`edit/${lateRecord.id}`} className="bg-blue-600 text-white px-4 py-2 rounded mr-2 hover:bg-blue-700 transition duration-300">Edit</Link>
                                                        <button className="bg-red-600 text-white px-4 py-2 rounded hover:bg-red-700 transition duration-300"
                                                            onClick={() => handleDelete(lateRecord.id)}>Hapus
                                                        </button>
                                                    </td>
                                                </tr>
                                            ))
                                                ...
                                                <td className="py-4 px-4 border-b text-center">
                                                    <div className="">
                                                        ...
                                                        {student.jumlah_keterlambatan >= 3 && (
                                                            <button onClick={() => handleDownloadPDF(student.student_id)} className="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 transition duration-300">
